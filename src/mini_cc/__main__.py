...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="mini-cc",
        description="Mini CC — a minimal coding agent",
        add_help=False,
    )
    ...
    parser.add_argument("--auto", action="store_true", help="Auto Mode: LLM classifier judges actions")
            ...
            if skill and skill.user_invocable:
                ...
                try:
                    if skill.context == "fork":
                        execute_skill(skill.name, cmd_args)
...
def main() -> None:
    ...
    if args.version:
        print(f"mini-cc version {__version__}")
        sys.exit(0)

    if args.help:
        print("""
Usage: mini-cc [options] [prompt]
...
Options:
  ...
  --model, -m         Model to use (default: claude-opus-4-6, or MINI_CC_MODEL env)
...
Examples:
  ...
  OPENAI_API_KEY=sk-xxx mini-cc --api-base https://aihubmix.com/v1 --model gpt-4o "hello"
  mini-cc --resume
  mini-cc  # starts interactive REPL
""")
        sys.exit(0)

    permission_mode = _resolve_permission_mode(args)
    model = args.model or os.environ.get("MINI_CC_MODEL", "claude-opus-4-6")
