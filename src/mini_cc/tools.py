"""Tool definitions and execution — 13 tools with 5 permission modes."""
...
tool_definitions: list[ToolDef] = [
    ...
    {
        "name": "agent",
        "description": "Launch a sub-agent to handle a task autonomously. Types: 'explore', 'plan', 'general'.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "tool_search",
        "description": "Search for available tools by name or keyword. Returns full schema definitions for matching deferred tools.",
        "input_schema": {
            "type": "object",
            "properties": {
...
def _grep_search(inp: dict) -> str:
    ...
    # Try system grep first (Linux/macOS, also available via Git on Windows)
    if not IS_WIN:
        try:
...
def _web_fetch(inp: dict) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "mini-cc/1.0"})
    try:
...
async def execute_tool(
    ...
    result = handler(inp) if not name == "list_files" else await handler(inp)
