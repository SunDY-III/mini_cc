...
class Agent:
    def __init__(
        ...
        *,
        ...
        # Message histories (separate for each backend)
        self._anthropic_messages: list[dict] = []
        self._openai_messages: list[dict] = []

        # Build system prompt with static/dynamic split
    ...
    async def run_loop(self, raw_input: str) -> None:
        ...
        if wants_cloud:
            print_info("(Real Claude Code would offer to convert this to a persistent cloud schedule. Continuing in-session.)")
    ...
    async def _run_loop_interval(self, spec: dict) -> None:
        print_info(
            f"⟳ /loop scheduled every {spec['interval_label']} (session-only). Ctrl+C to stop."
        )
        iterations = 0
    ...
    async def _run_loop_dynamic(self, spec: dict) -> None:
        print_info("⟳ /loop dynamic (self-paced) — Ctrl+C to stop.")
        ...
        try:
            while not self.loop_stop and not self._aborted:
                ...
                if not self.pending_wakeup:
                    ...
                    print_info(f"⟳ Loop converged after {iterations} tick{plural}.")
                    break
                budget = self._check_budget()
                if budget["exceeded"]:
                    print_info(f"Loop stopped: {budget['reason']}")
                    break
                if self.max_turns is not None and iterations >= self.max_turns:
                    print_info(f"Loop stopped: tick limit reached.")
                    break
                if iterations >= LOOP_MAX_ITERATIONS:
                    print_info(f"Loop stopped: reached {LOOP_MAX_ITERATIONS} ticks.")
                    break
    ...
    def _execute_schedule_wakeup(self, inp: dict) -> str:
        ...
        prompt_val = inp.get("prompt") if isinstance(inp.get("prompt"), str) else ""
        self.pending_wakeup = {"delay_seconds": delay, "reason": reason, "prompt": prompt_val}
    ...
    def _persist_large_result(self, tool_name: str, result: str) -> str:
        ...
        if len(result.encode()) <= THRESHOLD:
            return result
        d = Path.home() / ".mini-cc" / "tool-results"
        ...
        if name == "exit_plan_mode":
            ...
            if self._plan_approval_fn:
                ...
                if choice == "keep-planning":
                    ...
                    return f"User rejected the plan and wants to keep planning.\n\nUser feedback: {feedback}\n\nPlease revise your plan based on this feedback."
                ...
                if choice == "clear-and-execute":
                    ...
                    return f"User approved the plan. Context was cleared. Permission mode: {target_mode}\n\nPlan file: {saved_plan_path}\n\n## Approved Plan:\n{plan_content}\n\nProceed with implementation."

                print_info(f"Plan approved. Executing in {target_mode} mode.")
                return f"User approved the plan. Permission mode: {target_mode}\n\n## Approved Plan:\n{plan_content}\n\nProceed with implementation."
    ...
    async def _chat_openai(self, user_message: str) -> None:
        ...
        while True:
            ...
            for batch in oai_batches:
                ...
                if batch["concurrent"]:
                    async def _run_oai_safe(ct_item: dict) -> tuple:
